...
function base_versionsapi_compare($args)
{
    ...
    // parameter value or to NULL if nothing passed in at all.
    if (!isset($ver1)) {
    ...
    if($ver1 == NULL) {
        ...
        $msg = xarML('ver1 first version number');
    ...
    if($ver2 == NULL) {
        ...
        $msg = xarML('ver2 second version number');
        throw new EmptyParameterException($msg);
    }
    ...
    if (isset($validate)) {
        if (!xarMod::apiFunc('base', 'versions', 'validate', array('ver'=>$ver1, 'rule'=>$validate))
            || !xarMod::apiFunc('base', 'versions', 'validate', array('ver'=>$ver2, 'rule'=>$validate))
        ) {
            return false;
        }
    }
    ...
    if ($normalize) {
        list($ver1, $ver2) = xarMod::apiFunc('base', 'versions', 'normalize',
