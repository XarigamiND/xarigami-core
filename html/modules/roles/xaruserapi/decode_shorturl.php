...
function roles_userapi_decode_shorturl($params)
{
    ...
    if (empty($params[1])) {
        ...
        } elseif (!empty($params[2]) && preg_match('/^online/i',$params[2],$matches)) {
                $args['phase'] = 'active';
                if (!empty($params[3]) && preg_match('/^(\w+)/i',$params[3],$matches)) {
                    $args['letter'] = $matches[1];
                }  
    ...
    } elseif (preg_match('/^resetpassword/i',$params[1])) {
        return array('resetpassword', $args);
    ...
    } elseif (!empty($params[1]) && empty($params[2]) && xarModGetVar('roles','usernameurls')) {
        //let's check for username - flakey here but at this date, we don't have too much else available
        $user = xarMod::apiFunc('roles','user','get',array('uname'=>$params[1]));
        if (is_array($user)) {
            $uid = $user['uid'];
            $args['uid'] = $uid;
            return array('display',$args);
