...
/*
        if (xarCoreCache::isCached('DynamicData.SubForm',$name)) {
            $this->value = xarCoreCache::getCached('DynamicData.SubForm',$name);
            return true;
        }
*/
        ...
        if ($this->style == 'serialized') {
            $object = Dynamic_Object_Master::getObject(array('objectid'  => $this->objectid,
           ...
           $prefix = empty($this->fieldprefix) ? $name : $this->fieldprefix;
            ...
            if (empty($preview))
            {
                ...
                xarCoreCache::setCached('DynamicData.SubForm',$name,$value);
            }
            $this->value = $value;
            ...
            foreach ($object->properties as $property)
            {
                ...
                $propertyid = $property->id;
                 $prefix = empty($this->fieldprefix) ? $name : $this->fieldprefix;
                // check user input for the object item - using the current name as field prefix
                $propertyid = $prefix .'_dd_'.$propertyid;
...
//        } elseif ($this->style == 'childlist' && (empty($value) || $value == $oldvalue)) {
    ...
    function showInput(Array $args = array())
    {
        ...
        $data['fieldprefix'] = empty($this->fieldprefix) ? $name : $this->fieldprefix;
        if (!empty($this->objectid)) {
