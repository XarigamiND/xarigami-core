...
function main_120()
{
    $data['upgrade']['message'] = xarML('The upgrades for Xarigami Version 1.2.0 were successfully completed');
    $data['upgrade']['tasks'] = array();

    $upgrades = array(
                        'fix_120_01',   //Add new DD System Masks
                        'fix_120_02',   //Add new DD mod vars
                        'fix_120_03',   //Add installer theme
                    );
    foreach ($upgrades as $upgrade) {
        //check and then do upgrade if required
        $checkfile = str_replace('fix','check',$upgrade);
         if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_120/checks/'.$checkfile . '.php')) {
        ...
        if ($checkresult['test'] == 1 && $checkresult['success'] == 1) {
            ...
            if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_120/updates/' . $upgrade . '.php')) {
                $data['upgrade']['errormessage'] = xarUpgrader::$errormessage;
                return $data;
            }
            $result = $upgrade();
