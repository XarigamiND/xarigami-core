...
function main_133()
{
    $data['upgrade']['message'] = xarML('The upgrades for Xarigami Version 1.3.3 were successfully completed');
    $data['upgrade']['tasks'] = array();

    $upgrades = array(
                        'fix_133_01',   //Check themes table is updated with config column
                        'fix_133_02'    //check there is an index on theme vars table
                    );
    ...
    foreach ($upgrades as $upgrade) {
        ...
        $checkfile = str_replace('fix','check',$upgrade);
         ...
         if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_133/checks/'.$checkfile . '.php')) {
        ...
        if ($checkresult['test'] == 1 && $checkresult['success'] == 1) {
            ...
            if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_133/updates/' . $upgrade . '.php')) {
