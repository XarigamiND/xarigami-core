...
function main_150()
{
    $data['upgrade']['message'] = xarML('The upgrades for Xarigami Version 1.5.0 were successfully completed');
    $data['upgrade']['tasks'] = array();

    $upgrades = array(
                       'fix_150_01', //drop theme and module state tables, add xar_state field to module and theme table
                    );
    ...
    foreach ($upgrades as $upgrade) {
        ...
        $checkfile = str_replace('fix','check',$upgrade);
         ...
         if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_150/checks/'.$checkfile . '.php')) {
        ...
        if ($checkresult['test'] == 1 && $checkresult['success'] == 1) {
            ...
            if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_150/updates/' . $upgrade . '.php')) {
