...
function main_152()
{
    $data['upgrade']['message'] = xarML('The upgrades for Xarigami Version 1.5.2 were successfully completed');
    $data['upgrade']['tasks'] = array();

    $upgrades = array(
                 //      'fix_152_01', //session table field definition updates - session id and ip
                    );
    ...
    foreach ($upgrades as $upgrade) {
        ...
        $checkfile = str_replace('fix','check',$upgrade);
         ...
         if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_152/checks/'.$checkfile . '.php')) {
        ...
        if ($checkresult['test'] == 1 && $checkresult['success'] == 1) {
            ...
            if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_152/updates/' . $upgrade . '.php')) {
