...
function main_143()
{
    $data['upgrade']['message'] = xarML('The upgrades for Xarigami Version 1.4.3 were successfully completed');
    $data['upgrade']['tasks'] = array();

    $upgrades = array(
               //        'fix_143_01', //session table field definition updates - session id and ip
                    );
    ...
    foreach ($upgrades as $upgrade) {
        ...
        $checkfile = str_replace('fix','check',$upgrade);
         ...
         if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_143/checks/'.$checkfile . '.php')) {
        ...
        if ($checkresult['test'] == 1 && $checkresult['success'] == 1) {
            ...
            if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_143/updates/' . $upgrade . '.php')) {
