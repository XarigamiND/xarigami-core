...
function main_132()
{
    $data['upgrade']['message'] = xarML('The upgrades for Xarigami Version 1.3.2 were successfully completed');
    $data['upgrade']['tasks'] = array();

    $upgrades = array(
                        'fix_132_01',   //Check config.system.php is updated
    ...
    foreach ($upgrades as $upgrade) {
        ...
        $checkfile = str_replace('fix','check',$upgrade);
         ...
         if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_132/checks/'.$checkfile . '.php')) {
        ...
        if ($checkresult['test'] == 1 && $checkresult['success'] == 1) {
            ...
            if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_132/updates/' . $upgrade . '.php')) {
