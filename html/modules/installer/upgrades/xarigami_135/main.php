...
function main_135()
{
    $data['upgrade']['message'] = xarML('The upgrades for Xarigami Version 1.3.5 were successfully completed');
    $data['upgrade']['tasks'] = array();

    $upgrades = array(
                        'fix_135_01',   //update label for validation property is Configuration
    ...
    foreach ($upgrades as $upgrade) {
        ...
        $checkfile = str_replace('fix','check',$upgrade);
         ...
         if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_135/checks/'.$checkfile . '.php')) {
        ...
        if ($checkresult['test'] == 1 && $checkresult['success'] == 1) {
            ...
            if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_135/updates/' . $upgrade . '.php')) {
