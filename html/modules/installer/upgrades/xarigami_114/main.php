...
function main_114()
{
    $data['upgrade']['message'] = xarML('The upgrades for Xarigami Version 1.1.4 were successfully completed');
    $data['upgrade']['tasks'] = array();

    $upgrades = array(

                        'fix_114_01', // Update Roles mask for component All
                        'fix_114_02',  //convert module block priv, to block module privs!
                        'fix_114_03',  // Check correct block privs  for this Cumulus version
                                        //note that privs/role defaults have changed since 1.1.4
                        'fix_114_04',  //Removing Deny Block priv
                    );

    foreach ($upgrades as $upgrade) {
        //check and then do upgrade if required
        $checkfile = str_replace('fix','check',$upgrade);
         if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_114/checks/'.$checkfile . '.php')) {
        ...
        if ($checkresult['test'] == 1 && $checkresult['success'] == 1) {
            ...
            if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_114/updates/' . $upgrade . '.php')) {
