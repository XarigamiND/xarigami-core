...
{
    $data['check']['message'] = xarML('The checks for Xaraya version 1.1.3 were successful');
    $data['check']['tasks'] = array();

    $checks = array(
                        'check_113_01', //Disallowed emails moved back to Roles


                    );
    foreach ($checks as $check) {
        if (!xarUpgrader::loadUpgradeFile('upgrades/xaraya_113/checks/'.$check.'.php')) {
