...
function main_134()
{
    $data['upgrade']['message'] = xarML('The upgrades for Xarigami Version 1.3.4 were successfully completed');
    $data['upgrade']['tasks'] = array();

    $upgrades = array(
                        'fix_134_01',   //Check for charset in the config.system.php file
                        'fix_134_02'    //check there is a var/styles cache directory
                    );
    ...
    foreach ($upgrades as $upgrade) {
        ...
        $checkfile = str_replace('fix','check',$upgrade);
         ...
         if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_134/checks/'.$checkfile . '.php')) {
        ...
        if ($checkresult['test'] == 1 && $checkresult['success'] == 1) {
            ...
            if (!xarUpgrader::loadUpgradeFile('upgrades/xarigami_134/updates/' . $upgrade . '.php')) {
