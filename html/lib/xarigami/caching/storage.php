 ...
 */
sys::import('xarigami.caching.interfaces');
...
class xarCache_Storage extends xarObject
{
    ...
    public static function getCacheStorage(array $args = array())
    {
        ...
        {
            case 'database':
                sys::import('xarigami.caching.storage.database');
                $classname = 'xarCache_Database_Storage';
                break;

            case 'apc':
                if (function_exists('apc_fetch')) {
                    sys::import('xarigami.caching.storage.apc');
            ...
            case 'memcached':
                if (class_exists('Memcache')) {
                    sys::import('xarigami.caching.storage.memcached');
            ...
            case 'mmcache':
                if (function_exists('mmcache')) {
                    sys::import('xarigami.caching.storage.mmcache');
            ...
            case 'dummy':
                sys::import('xarigami.caching.storage.dummy');
...
// CHECKME: this will return false if the file doesn't exist yet - is that what we want here ?
    ...
    public function cleanCached($expire = 0)
    {
        ...
        if (!@touch($touch_file)) {
            ...
            error_log('Error from xarigami.caching.storage - web process can not touch ' . $touch_file);
        }
