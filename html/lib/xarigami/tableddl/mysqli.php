...
/**
 * Generate the MySQLi specific SQL to create a table
 *
 * @access private
 ...
 * @todo - recreate this specifically for mysqli
 */
function xarDB__mysqliCreateTable($tableName, $fields, $charset='utf8')
{
    $sql_fields = array();
    $primary_key = array();
 ...
 */
function xarDB__mysqliAlterTable($tableName, $args)
{
    switch ($args['command']) {
        case 'add':
            if (empty($args['field'])) {
       ...
       // TODO: adapt mysqlColumnDefinition to return field name too
            ...
            $coldef = xarDB__mysqliColumnDefinition($args['field'],$args);
            $sql .= $coldef['type'] . ' '
                . $coldef['unsigned'] . ' '
                . $coldef['null'] . ' '
                . $coldef['default'] . ' '
            ...
            //$sql .= join(' ', xarDB__mysqliColumnDefinition($args['field'], $args));
 ...
 * @todo DID YOU READ THE NOTE AT THE TOP OF THIS FILE?
 */
function xarDB__mysqliColumnDefinition($field_name, $parameters)
{
    $this_field = array();
    ...
    $this_field['primary_key'] = (isset($parameters['primary_key']) && $parameters['primary_key'] == true)
                               ...
                               : false;
    // Test for COMMENT
    $this_field['comment'] = (isset($parameters['coment']))
